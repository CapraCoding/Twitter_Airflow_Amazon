...
class TwitterHook(HttpHook):
    ...
    def create_url(self):
        ...
        start_time = (
            f"&start_time={(dt.datetime.strptime(self.start_time,'%Y-%m-%dT%H:%M:%S.00Z') - dt.timedelta(minutes=15)).strftime('%Y-%m-%dT%H:%M:%S.00Z')}"
            if self.start_time
            else ""
        )
        end_time = (
            f"&end_time={(dt.datetime.strptime(self.end_time,'%Y-%m-%dT%H:%M:%S.00Z') - dt.timedelta(minutes=15)).strftime('%Y-%m-%dT%H:%M:%S.00Z')}"
            if self.end_time
            else ""
        )
        url = "http://api.twitter.com/2/tweets/search/recent?query=(from:{})&{}&{}{}{}".format(
             query, tweet_fields, user_fields, start_time, end_time
        )
        print(url)
        return url
...
if __name__ == "__main__":
    for pg in TwitterHook("linkin_capra").run():
