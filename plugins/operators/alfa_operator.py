import sys
sys.path.append('/home/marcelo/Documentos/datapipeline/airflow/plugins')
import json
...
class AlfaOperator(BaseOperator):
    template_fields = [
        "query",
        "file_path",
        "start_time",
...
if __name__ == "__main__":
    with DAG(dag_id="TwitterTest", start_date=datetime.now()) as dag:
        to = AlfaOperator(
            query="AlfaConCP",
            file_path=join(
                "/home/marcelo/Documentos/datapipeline/datalake",
                "twitter_Alfa",
                "extract_date= {{ ds }}",
                "Alfa_{{ ds_nodash }}.json"
            ),
            task_id="test_run"
        )
